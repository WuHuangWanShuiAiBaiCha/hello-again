(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const frames = Array.from(document.querySelectorAll(".iphone-frame"));
    const getWidth = () => window.HelloAgain.utils.viewportWidth();
    const getLastPanel = () => {
      const rawValue = Number(refs.seriesShell?.dataset.panelLast);
      return Number.isFinite(rawValue) ? Math.max(1, Math.round(rawValue)) : 5;
    };
    ...
    function getCurrentFrameWindow() {
      const frame = frames[window.HelloAgain.utils.clamp(activePanel, 1, getLastPanel()) - 1];
      try {
    ...
    function disableFrameNavigation(frameDocument) {
      frameDocument.querySelectorAll("a[href], area[href]").forEach((link) => {
        if (!link.dataset.iphoneOriginalHref) {
          link.dataset.iphoneOriginalHref = link.getAttribute("href") || "";
        }
    ...
    function installFrameBridge(frame) {
      try {
        ...
        if (!frameWindow || !frameDocument || frameDocument.__iphoneSeriesBridgeInstalled) {
          return;
        }

        frameDocument.__iphoneSeriesBridgeInstalled = true;
      ...
      frameDocument.addEventListener("touchend", handleTouchEnd, { passive: true, capture: true });
        ...
        // iPhone pages are bundled same-origin files. Ignore future external frames.
      }
    }

    function primeIphoneTrackPosition() {
      if (!refs.seriesTrack) {
        return;
      }
